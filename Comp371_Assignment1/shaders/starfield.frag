...
uniform vec2 u_res;
uniform vec3 u_euler;

// fog density
float fDepthFade = 0.99;
...
void main()
{
	vec2 vScreenUV = gl_FragCoord.xy / u_res.xy;
	vec2 vScreenPos = vScreenUV * 2.0 - 1.0;

	vec3 vRayDir = normalize(vec3(vScreenPos, 1.0));

	vec3 vEuler = vec3(0.5 + sin(0 * 0.2) * 0.125, 0.5 + sin(0 * 0.1) * 0.125, u_time * 0.1 + sin(0 * 0.3) * 0.5);
	vEuler = vec3(0,0,0);

	vEuler.x = -(((u_euler.y) * 2.0 - 1.0)) / 200;
	vEuler.y = -(((u_euler.x) * 2.0 - 1.0)) / 200 - 120.3;
	vEuler.z = 0.0;
